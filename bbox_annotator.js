...
(function() {
  ...
  BBoxSelector = (function() {
    ...
    BBoxSelector.prototype.create_label_box = function(options) {
      var $input, input, j, k, label, len, len1, ref, ref1, select1, select2;
      ...
      switch (options.input_method) {
        ...
        case 'select2':
          ...
          this.label_input = $('<div class="multi-select"><select class="label_input" name="label"></select><select class="label_input" name="label2"></select></div>');
          select1 = this.label_input.find('select').eq(0);
          select2 = this.label_input.find('select').eq(1);
          this.label_box.append(this.label_input);
          ref1 = this.label_input.find('select');
          ...
          select2.change(function(e) {
            return $(this).parent().blur();
          });
          select1.change(function(e) {
            var l, label1, len2, ref2, results;
            select2.html('<option value>choose an item</option>');
            label1 = select1.val();
            ref2 = options.labels[label1];
            results = [];
            for (l = 0, len2 = ref2.length; l < len2; l++) {
              label = ref2[l];
              results.push(select2.append('<option value="' + label + '">' + label + '</option>'));
            }
            return results;
          });
          break;
    ...
    BBoxSelector.prototype.finish = function(options) {
      ...
      this.selector.hide();
      data = this.rectangle();
      if (this.label_input.hasClass('multi-select')) {
        data.label = this.label_input.find('select').map(function(i, select) {
